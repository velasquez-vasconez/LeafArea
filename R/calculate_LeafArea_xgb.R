#' Calculate Leaf Area using XGBoost Model
#'
...
#' @importFrom caret trainControl train defaultSummary R2
...
calculate_LeafArea_xgb <- function(data) {
  ...
  params <- list(
    ...
    eval_metric = "rmse",  # Métrica de evaluación (Root Mean Squared Error)
  ...
  controle <- caret::trainControl(
    method = "cv",
    number = 10,
    summaryFunction = defaultSummary,  # Função de avaliação de performance padrão para regressão
    classProbs = TRUE  # Não é necessário para regressão
  )

  treino$sp <- as.factor(treino$sp)
  set.seed(5678)
  modelo_xgb <- caret::train(
    LeafArea  ~ LeafLength + LeafWidth + sp,
    data = treino,
    method = "xgbTree",
    trControl = controle,
  ...
  predicciones <- predict(modelo_xgb, teste)
  ...
  predicciones <- predict(modelo_xgb, treino)
  ...
  data$LeafArea_xgb_model <- predict(modelo_xgb, newdata = data)

  cat("\n---------------------------------------------\n\n")

  cat("Performance Metrics of XGBoost model:\n")
  print(modelo_xgb)
